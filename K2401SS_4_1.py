...
# V4.1 12/30/23. Several small updates:
#     1. Add Sample_Name, Cell_Number_ and Scan_Type columns to the parameters 
#        and timeseries_parameters files in calc_and_save_parameters().
#     2. Removed the trailing "F" or "R" from the Cell_Number column data. 
#        I think this change is in the Jupyter notebook when calc_and_save_parameters() is called.
#     3. Choose 4W or 2W using the new four_wire_mode parameter in IVSweep().
#     4.  Slow sweep mode now has delay set to 1.0.
#     5. Save the time in both the parameters and timeseries_parameters files. 
#     6. Add a column for measurement type.  These may ibe "F", "R", or "S" for stabilized. 
#     7. Rename Sample_Cell# to Test_ID in preparation for saving to a database. 
# 
"""
...
class K2401 functions:
    ...
    IVsweep(self, startv, stopv, stepv, rate="medium", final_V="0", 
    mpp_tracking_mode=False, four_wire_mode=False) : perform a voltage
    sweep while measuring current.  Rates of 'slow', 'medium' and 'fast'
    are suppored. They correspond to 0.2, 0.1, and 0.0 line voltage cycle
    delay between each data point. The marameter final_V controls the voltage
    that the sourcemeter is set at at the conclusion of the scan.  

    dark_light_IVsweep(self, startv, stopv, stepv, rate="medium", 
    area=1.0, plottitle="", verbose=1,labjack='True', burnintime=1,
    do_dark=True, mpp_tracking_mode=False, four_wire_mode=False) : Call IVsweep twice, once with
    ...
    after subsequent measurement. The column 'Sample_Cell#" contains an 
    abbreviated string composed of the sample name, the cell number, and the
    scan type ('F' for Forward, 'R' for Reverse, or 'S' for Stabilized). 
        If timeseries==True, a special timeseries_parameters file is 
    created with an extra column, 'Elapsed Time', which records the 
    time of each measurement relative to the time that the '/data' 
    folder was created. Note: In V4.1, the regular parameters file also 
    has 'Elapsed Time'. Also in V4.1, the parameters or timeseries_parameters 
    file has columns for 'Sample_Name', 'Cell_Number', and 'Scan_Type'.  
    These contain the same information as 'Sample_Cell#', but 
    they are separate instead of all being appended into one string. I
    decided not to eliminate the 'Sample_Cell#' column for the sake of
...
class dummy_LabJackU3:
    ...
    The user is prompted to switch the lamp on and off manually.
"""
class K2401:
    import matplotlib.pyplot as plt
    from scipy import stats

    def __init__(self, dll_dir=r'C:\\Windows\system32\visa64.dll', GPIB_channel="GPIB::0::27"):
        """
        ...
        for key, value in rm.list_resources_info().items():
            ...
            i += 1
        # Open the device. Use GPIB_channel to set the channel.
        ADDRESS = GPIB_channel
        ...
        # For mpp tracking
        self.mpp_V = "0" # Just a default value.  Will be updated after each sweep.
        return 

    def version():
        print("K2401 package V4.1 12/30/2023")


    #===============    IVsweep function   ===============#

    def IVsweep(self, startv, stopv, stepv, rate="fast", final_V="0", 
                mpp_tracking_mode=False, four_wire_mode=False):
        from numpy import arange, ceil
        from pandas import DataFrame
        """
        Source and sweep the voltage, measure the current.
        Return a dataframe with the Voltage and Current data.
        """
        # IV Sweep for 2400 SourceMeter
        ...
        if (rate == "slow"):
            ...
            delay = "0.0" # fast is the default

        # Variable assignment
        steps = str(int(ceil((stopv - startv) / stepv)))
        ...
        self.ctrl.write(":SOUR:SWE:POIN ", steps)
        self.ctrl.write(":SOUR:SWE:DIR UP")
        self.ctrl.write(":TRIG:COUN ", steps)
        ...
        # Set 2-wire or 4-wire mode.
        if four_wire_mode:
            self.ctrl.write(":SYSTEM:RSENSE 1")
        else:
            self.ctrl.write(":SYSTEM:RSENSE 0")

        # Turn beep off 
        self.ctrl.write(":SYST:BEEP:STAT 0")
        self.ctrl.write(":OUTP ON")

        # Initiate sweep, collect ACSII current values, and turn output off
        result = self.ctrl.query(":READ?")
        self.ctrl.write(":SYST:BEEP:STAT 1")
        ...
        if(mpp_tracking_mode != True):
            # turn off unless we are tracking mpp
     ...
     #===============         dark_light_IVsweep function           ===============#
    
    def dark_light_IVsweep(self, startv, stopv, stepv, rate="medium", 
                            area=1.0, plottitle="", verbose=1,labjack='True', burnintime=1,
                            do_dark=True, mpp_tracking_mode=False, four_wire_mode=False):
        ...
        if(mpp_tracking_mode):
            dfl = self.IVsweep(startv, stopv, stepv, rate=rate, final_V=self.mpp_V, 
                mpp_tracking_mode=True, four_wire_mode=four_wire_mode)
        else:
            dfl = self.IVsweep(startv, stopv, stepv, rate=rate, four_wire_mode=four_wire_mode)
        stop_t = time.time()
        if(verbose>0):
            print("Sweep rate: {:3.1f} mV/sec".format(1000*(stopv-startv)/(stop_t-start_t)))
     ...
     #===============         save_data-and_plot function           ===============# 

    def save_data_and_plot(self, df, fig, path = r'C:\\Users\Public',
                            samplename = "testymctest", cellnumber = "0",  timenowstr = "", verbose="yes"):
        ...
        return filename + ".csv", filename + ".png"

    def calc_and_save_parameters(self, df,  path = r'C:\\Users\Public',
        ...
        v = df[samplename+"_"+cellnumber + scantype + "_V_L"]
        j = df[samplename+"_"+cellnumber + scantype + "_J_L"]
        #
        ...
        try:
            fv = interp1d(j,v,kind='linear')
            v_oc = fv(0.0)
            jstep = 1e-6
            r_ser = (fv(jstep)-fv(0.0))/jstep
            ff = 100.0*pmax/(v_oc*j_sc)
        except:
        ...
        # 
        if (verbose == 1):
            print("I_ph = {:5.3f} suns".format(I_ph))
        ...
        #
        #### Save parameters
        sample_cellnum_time_str = samplename +"_"+ cellnumber + scantype + "_"+ timenowstr
        if (timenowstr == ""):
            timenow = datetime.now()
            timenowstr = timenow.strftime("%H%M%S")
        if (timeseries == True):
        ...
        data = {"Elapsed_Time": (time.time() - base_t)/60.0,
                "Sample_Name": samplename,
                "Cell_Number": cellnumber,
                "Scan_Type": scantype,
                "Test_ID":  sample_cellnum_time_str,
                "I_ph(suns)": [I_ph],
                "V_oc(V)":  [v_oc],
                "mpp(V)":  [mpp],
                "J_sc(mA)":  [j_sc],
