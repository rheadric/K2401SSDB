...
$deleteCreatedRecords = $false # change this if you want to keep the records created by this sample
...
Invoke-DataverseCommands {
   ...
   #region Section 1b: Basic Create and Update operations
   Write-Host "`n--Section 1b started--`n"

#    $testContactRafelShillo = @{
#       'firstname' = 'Felix'
#       'lastname'  = 'Walberg'
#    }
   $ern = @{
...
#    $testId = New-Record `
#       -setName 'pets' `
#       -body $testContactRafelShillo
   $ernID = New-Record `
    -setName 'cr69a_pets' `
    -body $ern

#    Write-Host "Contact '$($testContactRafelShillo.firstname) $($testContactRafelShillo.lastname)' created"
#    Write-Host "`tContact URI: $baseURI($testId)"
#    Write-Host "`tContact relative Uri: contacts($testId)"

   Write-Host "Pet '$($ern.cr69a_petname)' the '$($ern.cr69a_breed)' created"
   Write-Host "`Pet URI: $baseURI($ernId)"
   Write-Host "`Pet relative Uri: cr69a_pets($ernId)"

   # To delete later
#    $recordsToDelete += [PsObject]@{
#       'description' = "Contact '$($testContactRafelShillo.firstname) $($testContactRafelShillo.lastname)'"
#       'setName' = 'pets'
#       'id'      = $testId
#    }
   $recordsToDelete += [PsObject]@{
    'description' = "Pet '$($ern.cr69a_petname) $($ern.cr69a_breed)'"
    'setName' = 'cr69a_pets'
    'id'      = $ernID
 }

   # Data to update for Rafel Shillo
#    $testRafelShilloUpdate1 = @{
#       'annualincome' = 80000
#       'jobtitle'     = 'Junior Developer'
#    }
   # Data to update for Ernie
   $ernUpdate1 = @{
    'cr69a_appointmentdate' = 'November, 25, 2003, 5:00 am'
    }
   ...
   $message = "Pet '$($ern.cr69a_petname) $($ern.cr69a_breed)' "
   $message += 'updated with owner.'
   Write-Host $message

   # Retrieve the created contact
#    $testRetrievedRafelShillo1 = Get-Record `
#       -setName 'pets' `
#       -id $testId `
#     #   -query '?$select=fullname,annualincome,jobtitle,description'
    $retrieveErn = Get-Record `
        -setName 'cr69a_pets' `
        -id $ernID `
        -query '?$select=cr69a_petname,cr69a_species,cr69a_breed,ownerid'

#    Write-Host "`nContact '$($testRetrievedRafelShillo1.fullname) retrieved."
#    Write-Host "`tAnnual income:" `
#    ($testRetrievedRafelShillo1.'annualincome@OData.Community.Display.V1.FormattedValue')
#    Write-Host "`tJob title:" `
#    ($testRetrievedRafelShillo1.jobtitle)
#    Write-Host "`tDescription:" `  # Description is initialized empty.
#       ($testRetrievedRafelShillo1.description)

    Write-Host "`nPet '$($retrieveErn.cr69a_petname) retrieved."
    Write-Host "`tBreed: '$($retrieveErn.cr69a_breed)'"

   # Modify specific properties and then update contact record.
#    $testRafelShilloUpdate2 = @{
#       'jobtitle'     = 'Senior Developer'
#       'annualincome' = 95000
#       'description'  = 'Assignment to-be-determined'
#    }
# TODO: Do this for ern

   # Update record with new values
#    Update-Record `
#       -setName 'pets' `
#       -id $testId `
#       -body $testRafelShilloUpdate2
# TODO: Do this for ern


#    Write-Host "Contact '$($testRetrievedRafelShillo1.fullname)' updated"
#    Write-Host "`tJob title: $($testRafelShilloUpdate2.jobtitle)"
#    Write-Host "`tAnnual income: $($testRafelShilloUpdate2.annualincome)"
#    Write-Host "`tDescription: $($testRafelShilloUpdate2.description)"
# TODO: Do this for ern
...
#       -id $ernID `
#       -property 'ownerid' `
#       -value 'Mia Twarog' | Out-Null

   Write-Host "`nPet '$($retrieveErn.cr69a_petname)' owner updated."
   
   # Retrieve just the owner column
   $ernOwner1 = Get-ColumnValue `
      -setName 'cr69a_pets' `
      -id $ernID `
      -property 'ownerid'

   Write-Host "`tPet's owner is: $ernOwner1"

   #endregion Section 1b: Basic Create and Update operations
