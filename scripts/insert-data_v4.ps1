...
param (
    ...
    [Parameter(Mandatory=$true)]  [string] $scan_data_path,   # 22
...
Invoke-DataverseCommands {
    ...
    Write-Host "mpp: '$($mpp_v)'"
    Write-Host "four wire mode: '$($four_wire_mode)'"
    ...
    if ($four_wire_mode -like 'False') {$four_wire_mode = $false }
    ...
    $cell = @{
            ...
            $colArray[3] =     $test_id 
            $colArray[4] =     [decimal]$i_ph_suns
            $colArray[5] =     [decimal]$voc_v
            $colArray[6] =     [decimal]$mpp_v
            $colArray[7] =     [decimal]$jsc_ma
            ...
            $colArray[19] =    [decimal]$temp_c
            $colArray[20] =    [decimal]$hum_pct
            $colArray[21] =    [bool]$four_wire_mode
            $colArray[22] =    $scan_data_path
           }
...
####### The code below is no longer needed. #######
    ...
    # Write-Host "Type of `$recordId: $($recordId.GetType())"
...
#######  The code below gives an error, so I have commented it out for now.  #######
    ...
    #     $cell['ccr69a_sampledatav2lookup.cr69a_sample_id'] = $lookupPayload
    #     Write-Host "cell: '$($cell['cr69a_sampledatav2lookup.cr69a_sample_id'])'"
    # }
